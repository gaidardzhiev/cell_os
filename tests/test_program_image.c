...
ok&=must(cell_task_run(&tm,&vfs,&env,"/programs/hello",out,sizeof(out),&id),"run hello",out);ok&=must(strcmp(out,"Hello from CellExec-1.\n")==0,"hello output",out);
ok&=must(cell_task_run(&tm,&vfs,&env,"/programs/observe",out,sizeof(out),&id),"run observe",out);ok&=must(strstr(out,"CellExec observation:\nCell OS is running.")==out,"observe system",out);ok&=must(strstr(out,"Memory:")!=0,"observe memory",out);
